...
## Quick Start (Run using Test data)

This project already includes a sample dataset located in `data/csv/test_dataset.csv`.
Use the commands below to verify that data processing and inference work correctly without modifying anything.

### 1. Process dataset for inference
```
python scripts/data_processing.py --mode inference --input_file ../data/csv/test_dataset.csv
```
Expected Output: `data/processed/test_dataset_processed.parquet`
### 2. Run inference using the pre-trained model and scaler
```
python scripts/inference.py --input_file ../data/processed/test_dataset_processed.parquet
```
Expected Output: `data/inference/test_dataset_inference.csv`
...
```
Run all cmds from home location thanks.
---
